...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === "FETCH_CAPTIONS_API_RESULT") { // Assuming background sends this type
    console.log("Received captions from background:", request.captions); // Log the received data
    sendResponse({ captions: request.captions }); // Send back to popup
  } else if (request.type === "FETCH_CAPTIONS_API_ERROR") {
    console.error("Error fetching captions in content:", request.error);
    sendResponse({ captions: [] }); // Send back an empty array or error indication
  }
  return true; // Keep channel alive if needed
...
// Bookmark button handler: Send message to popup logic
